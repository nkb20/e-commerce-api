...
const { handleValidationError, handleDuplicateError, handleError } = require("../custom_error/errors")


const handleUserRegistration = async function (req, res, next) {
  const new_user = req.body;
  var role = userRole.USER;
  if (Object.values(userRole).includes(new_user.role)) {
    role = new_user.role;
  }
  ...
  try {
    ...
    var errorArray = [];
    errorArray = errorArray.concat(handleValidationError(err));
    errorArray = errorArray.concat(handleDuplicateError(err));
    errorArray = errorArray.concat(handleError(err, next));
...
const handleUserLogin = async (req, res, next) => {
  ...
  try {
    ...
    if (isPassword) {
      ...
      log.info(`Token generated for userId ${user.user_id}`)
...
const handleChangePassword = async (req, res, next) => {
  ...
  try {
    ...
    log.info(`password has been changed for user id=${user._id}`)
